...
- `oled_ui.*`：三页轮换显示 PC 硬件、Codex 配额和路由检测设备信息；长名称在限定区域内往返滚动。Codex flags bit2 置位时继续显示最后有效额度，并标记 `STALE`。
...
Codex payload schema 仍为 v2、固定27字节。flags bit2 用作 `STALE` 标志：PC 上位机启动时可先发送14天内的最近有效缓存；额度查询暂时失败时也保留该值。OLED 标题显示 `CODEX WEEKLY STALE`，收到新的有效额度后恢复正常。`AuthRequired` 不使用缓存掩盖，仍显示 `LOGIN REQUIRED`。
